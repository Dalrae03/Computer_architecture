...
<ins>받아들이는 정보</ins>: 레지스터를 통해 피연산자, 제어장치로 부터 제어신호

<ins>내보내는 정보</ins>: 레지스터에 결괏값, 플래그 레지스터에 플래그
...
<ins>받아들이는 정보</ins>: 클럭 신호, 명령어 레지스터에서 해석할 명령어, 플래그 레지스터에서 플래그 값, 제어버스를 통해 외부로 부터 전달된 제어 신호 등

<ins>내보내는 정보</ins>: CPU외부에 전달하는 제어신호, CPU내부에 전달하는 제어신호

<br>

<ins>CPU외부에 전달하는 제어신호 </ins> ⇒ 제어버스로 제어신호를 내보낸다. (메모리에 전달, (보조기억장치 포함) 입출력장치에 전달)

<ins>CPU내부에 전달하는 제어신호</ins> ⇒ ALU 전달 제어 신호 (수행할 연산 지시), 레지스터에 전달하는 제어 신호 (레지스터 간 데이터 이동시키거나 레지스터에 저장된 명령어를 해석하기 위해).
...
- 순서
    1. 입출력장치가 CPU에 <ins>인터럽트 요청신호</ins>를 보냄
    2. CPU는 실행 사이클이 끝나고 명령어를 인출하기 전 항상 인터럽트 여부 확인
    3. CPU는 인터럽트 요청 확인. <ins>인터럽트 플래그</ins>를 통해 현재 인터럽트를 받아들일 수 있는지 여부 확인
    4. 인터럽트를 받아들일 수 있다면 CPU는 지금까지의 작업을 백업
    5. CPU는 <ins>인터럽트 벡터</ins>를 참조하여 <ins>인터럽트 서비스 루틴</ins>을 실행
    6. 인터럽트 서비스 루틴 실행이 끝나면 4에서 백업해 둔 작업을 복구하여 실행을 재개

<br>

- <ins>인터럽트 플래그</ins> - 하드웨어 인터럽트를 받아들일지, 무시할지를 결정하는 플래그
...
- <ins>인터럽트 서비스 루틴 (인터럽트 핸들러)</ins> - 인터럽트를 어떻게 처리하고 작동해야 할지에 대한 정보로 이루어진 프로그램 (명령어와 데이터로 이루어져있다 → 프로그램 카운터를 비롯한 레지스터들을 사용하며 실행된다)
